...
//        GET /v1/resource/{resourceId}/latest Retrieve latest values of a Resource
    ...
    public String getResourceSummary(Long resourceId,String info) throws Exception,AssertionError {
...
//        TODO same out for adding Uom with ID
        try {
            ...
            String response = given()
                    ...
                    .extract()
                    .body().asString();
            if ("min" == info || "max" == info || "average" == info) {
                Map<String, Integer> data = from(response).getMap(info);
//                System.out.println("Resource " + resourceId + " in  \"" + info + "\": " + data);
            }
//            else {
//                List<Long> data = from(response).getList(info);
//                System.out.println("Resource " + resourceId + " in  \"" + info + "\": " + data);
//            }
            String room_id = from(response).get("keyName").toString().split("/")[1]; //  "keyName": "0013a20040a1d47f/0x4fd/temp",
            return room_id;
        }
        catch(AssertionError e){
